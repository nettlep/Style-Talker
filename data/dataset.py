...
class DataCollatorStyles(object):
    ...
    def __call__(self, instances: Sequence[Dict]):
        ...
        # Add styles to batch
        in_styles = [instance['in_styles'] for instance in instances]
        tar_style = torch.stack([instance['tar_style'] for instance in instances], 0)
        audio_info = [instance['audio_info'] for instance in instances]
...
class DataCollatorDefault(object):
    """Collate examples for supervised fine-tuning."""
    def __init__(self, tokenizer: transformers.PreTrainedTokenizer):
        self.tokenizer = tokenizer
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            if role == '<|im_start|>user':
                _audio_info = tokenizer.process_audio(sentence["value"])
                _input_id = tokenizer(role).input_ids + nl_tokens + \
                    tokenizer(sentence["value"], audio_info=_audio_info).input_ids + [im_end] + nl_tokens
            else:
                _input_id = tokenizer(role).input_ids + nl_tokens + \
        ...
        if len(input_id) > max_len:
            print(f'input len {str(len(input_id))} > max len {str(max_len)}!!!') 
            return None
        input_id += [tokenizer.pad_token_id] * (max_len - len(input_id))
        target += [IGNORE_TOKEN_ID] * (max_len - len(target))
        input_ids.append(input_id[:max_len])
        targets.append(target[:max_len])
        audio_info.append(_audio_info)
...
class StyledConversations(Dataset):
    ...
    def __init__(
        ...
        super(StyledConversations, self).__init__()
        ...
        self.style_in_id = self.tokenizer.convert_tokens_to_ids('<|extra_123|>')
        self.style_out_id = self.tokenizer.convert_tokens_to_ids('<|extra_124|>')
    ...
    def __getitem__(self, i, 
        ...
        # sample a different style from the target as the reference, 
        ...
        if ret == None:
            print(f'Maximum token length {str(self.max_len)} exceeded! Resample an input.')
